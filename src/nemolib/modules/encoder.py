...
class CrossViewEncoder(nn.Module):
    def __init__(
        ...
        self.transformers_cross, self.transformers_self = make_encoder_transformer_layers(
            in_dim, layers, norm_first, use_flash_attn
        )

        # 2d positional embedding
        embedding_stdev = 1.0 / math.sqrt(in_dim)
        ...
        self.cam_emb = nn.parameter.Parameter(torch.rand(32, in_dim, 1, 1) * embedding_stdev)
...
def make_encoder_transformer_layers(in_dim, layers, norm_first: bool = False, use_flash_attn: bool = False):
    ...
    if not use_flash_attn:
        ...
        for _ in range(num_layers):
            cross_attn = TransformerCrossAttnLayer(
                d_model=in_dim,
                nhead=8,
            ...
            self_attn = torch.nn.TransformerEncoderLayer(
                d_model=in_dim,
                nhead=8,
                dim_feedforward=latent_dim,
                dropout=0.0,
